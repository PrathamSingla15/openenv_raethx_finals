...
<h2 class="tb-section-title">
    ...
    <code>available_at &le; t</code>.
</h2>

<figure class="tb-figure">
    <div class="tb-figure-head">
        <div class="tb-figure-title">temporal contract &middot; t &rarr; t+1</div>
        <div class="tb-figure-meta">5 phases &middot; 5 gates</div>
    </div>
    <div class="tb-figure-body is-dark" style="padding: 32px 24px;">
        <svg viewBox="0 0 880 220" style="width: 100%; height: auto;" role="img" aria-label="Temporal contract diagram">
            ...
            <g font-family="IBM Plex Mono, monospace" font-size="11">
                ...
                <text x="120" y="80" text-anchor="middle" fill="rgba(255,255,255,0.66)" font-size="10">data &le; t</text>
                ...
                <text x="600" y="94" text-anchor="middle" fill="#FFFFFF" font-size="10">7 components &times; gate</text>
                ...
                <text x="760" y="80" text-anchor="middle" fill="rgba(255,255,255,0.66)" font-size="10">data &le; t+1</text>
...
<div class="tb-layer-stack">
    <div class="tb-layer">
        ...
        <div class="tb-layer-content">
            ...
            <div class="tb-layer-body">Every DuckDB query filters
                <code>available_at &le; current_date</code>. The data layer never
    ...
    <div class="tb-layer">
        ...
        <div class="tb-layer-content">
            ...
            <div class="tb-layer-body">Only files with
                <code>available_at &le; current_date</code> are materialized into the
    ...
    <div class="tb-layer">
        ...
        <div class="tb-layer-content">
            ...
            <div class="tb-layer-body">Agents see opaque <code>tier_a01</code> &hellip;
                <code>tier_a10</code> IDs against a randomly drawn source window.
                Per-build alias-to-ticker permutation + &sigma;=0.0005 return-noise
...
<h2 class="tb-section-title">
    ...
    reach the agent</span> &mdash; and the gate that closes it.
</h2>
...
<table class="tb-table">
    ...
    <tbody>
        <tr><td>Daily OHLCV</td><td class="mono">data/query.py:get_bars</td><td>SQL <code>available_at &le; ?</code> + tool-level <code>as_of_date</code> gate</td></tr>
        <tr><td>Universe membership</td><td class="mono">data/query.py:load_universe</td><td>SQL + tool-level gate</td></tr>
        <tr><td>Symbol &harr; asset map</td><td class="mono">data/query.py:get_symbol_to_asset_id</td><td>SQL + tool-level gate</td></tr>
        ...
        <tr><td>RNG</td><td class="mono">session.py:_rng</td><td>Seeded from manifest digest or <code>reset(seed=&hellip;)</code></td></tr>
    </tbody>
</table>
"""
...
<ul style="line-height: 1.8; max-width: 64ch; color: var(--ink-mute); padding-left: 24px;">
    <li><code>scan_forbidden_globals</code> &mdash; regex scanner over every
        <code>sandbox_exec</code> stdout/stderr; 12 patterns including
        <code>eval(</code>, <code>__import__</code>, <code>subprocess.</code>,
        <code>socket.</code>, ledger private-attr writes. Hit &rarr;
        <code>viol_hack = 1</code>, gate = 0 on the next <code>advance_day</code>.</li>
    <li><code>check_rules_clause</code> &mdash; regex scanner over the agent's
        ...
        Hit &rarr; <code>viol_rules = 1</code>, gate = 0.</li>
    <li><strong>Bounded components.</strong> Every <code>c<sub>i</sub></code> &isin; [0, 1] by
...
_VERIFIER_HTML = """
<div class="tb-section-eyebrow">verifier &middot; run after every change</div>
<h2 class="tb-section-title">
...
<div class="tb-verifier-strip">
    <span class="tb-vcheck">PASS &middot; conformance.observation</span>
    <span class="tb-vcheck">PASS &middot; conformance.action_rejection</span>
    <span class="tb-vcheck">PASS &middot; leak.fs</span>
    <span class="tb-vcheck">PASS &middot; leak.no_future_fit</span>
    <span class="tb-vcheck">PASS &middot; replay</span>
    <span class="tb-vcheck">PASS &middot; determinism</span>
</div>

<details class="tb-schema">
    <summary>$ python -m tradebench.verifier --tier test --seed 42 --max-bars 5</summary>
<pre>TradeBench verifier &mdash; tier=test seed=42
  ...
  PASS  determinism                   |&Delta;V| = 0.000e+00 &lt; 1e-06; 11 events; 10 actions
RESULT: PASS</pre>
</details>
"""
